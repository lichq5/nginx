...
/*
    if (commu == NULL) {
        ...
        if ( !(commu->initialize(80)) )
            return NGX_ERROR;
    }

    CommData* reqMessage = commu->createMessage();
    CommData** request = (CommData **)new CommData**;
    (*request) = reqMessage;
    if (!(commu->sendMessage(reqMessage))) {
        return NGX_HTTP_NOT_ALLOWED;
    }
*/
/*    if (commu->sendAndWait(request) == NULL) {
...
*/
    ...
    //要打开的文件
    u_char* filename = (u_char*)"/home/nico/slam_resource/tb_condo_2.pgm";
    b->in_file = 1;
